...
const New = (props) => {
    ...
    let data = useSelector(state => state.iblock.detailNew);
    useEffect(() => {
        let breads = [
            {id: 1, name: 'Главная', link: '/'},
            {id: 2, name: 'Новости', link: '/novosti'},
    ...
    useEffect(() => {
        const getOneNewFunction = async () => {
            let result = await dispatch(getEntity({id: 1, link: params.id}));
            if (!result.success) {
                history.push('/404');
            }
        }
    ...
    let type = data.properties && data.properties.length && data.properties.find(item => item.userFieldId === 2);
    return (
        <div className="block new">
            ...
            {data.text && <div className="new__body main__col" dangerouslySetInnerHTML={{__html: data.text}}></div>}
            {data.next && <More text={data.next.name} link={'/novosti/' + data.next.linkName} />}
        </div>
    );
}
